...
<body id="body" class="wrapper">
 ...
 <!-------------------------------------------------找回密码第一步star ---------------------------------------------------------->
  ...
  <div class="asRetrieve_pwContent"><p class="sprite_a pwContentTitle02"><span class="asfontnone">1.输入用户名</span></p>
  <span class="box30"></span>
        ...
        <ul id="asRetrieve_pwList">
         <li>
          <center>
            @if (session('su'))
            <span style="color:green;font-size:24px;">{{session('su')}}邮件发送成功,请去邮件中查看</span>
            @endif
            @if (session('error'))
            <span style="color:red;font-size:24px;">{{session('error')}}请点击浏览器的退回到上一步</span>
